# Vue 3 企業級完整架構專案 + Vue Query & VueUse

一個基於 Vue 3 + Pinia + Zod + Vue Query + VueUse + JSON Server 的完整企業級前端架構示範專案。

## 🆕 最新更新 - Vue Query & VueUse 整合
...
┌─────────────────────────────────────────────────────┐
│  🖼️ View Layer (組件)                                │
│  PostDetail.vue, UserList.vue, DataDemo.vue        │
└────────────────┬────────────────────────────────────┘
                 │ 使用
┌────────────────▼────────────────────────────────────┐
│  🪝 Hooks Layer (Vue Query & VueUse)                │
...
│  composables/useUserForm.js, useAuth.js            │
└────────────────┬────────────────────────────────────┘
                 │ 使用
┌────────────────▼────────────────────────────────────┐
...
┌────────────────▼────────────────────────────────────┐
│  🔌 API Layer (API 函數)                             │
│  services/api/user.js, services/api/post.js        │
│  - 定義 API 端點                                     │
│  - Zod 驗證                                          │
└────────────────┬────────────────────────────────────┘
                 │ 使用
...
│   └── index.js
├── views/                  # 頁面組件
│   ├── DataDemo.vue        # 🆕 Vue Query & VueUse 示範頁面
...
export function useUsers() {
  const { data, isFetching, error, execute } = useFetch('/api/users', {
    afterFetch(ctx) {
      ctx.data = parseUsers(ctx.data) // 自動驗證
      return ctx
    },
  }).json()
